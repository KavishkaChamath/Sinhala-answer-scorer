...
OLLAMA_MODEL = "gemma3:4b"  # Change to any model you have pulled: llama3.2, mistral, gemma2, etc.
...
class ScoringAgent:
    ...
    def build_prompt(
        ...
        criteria_text = "\n".join(
            f"  {c['id']}: {c['description']} [max {c['max_marks']} marks]"
            for c in guide["criteria"]
        )

        prompt = f"""You are an expert examiner for Sri Lankan History. 
You are grading a student's answer written in Sinhala.
Score the answer strictly based on the marking guide provided.

=== QUESTION ===
{guide['question_en']}
(Sinhala: {guide['question_si']})
...
=== ONTOLOGY CONCEPTS FOUND IN ANSWER ===
{ontology_enrichment}

=== STUDENT ANSWER (in Sinhala) ===
{student_answer}

=== INSTRUCTIONS ===
1. Read the student answer carefully (it is in Sinhala).
2. For each marking criterion, decide how many marks to award (0 to max_marks).
3. Provide brief evidence-based justification for each score.
4. Output ONLY valid JSON in the exact format below. No extra text.

{{
  "criteria_scores": [
    {{"id": "C1", "awarded": <int>, "max": 4, "justification": "<string>"}},
    {{"id": "C2", "awarded": <int>, "max": 4, "justification": "<string>"}},
    {{"id": "C3", "awarded": <int>, "max": 4, "justification": "<string>"}},
    {{"id": "C4", "awarded": <int>, "max": 4, "justification": "<string>"}},
    {{"id": "C5", "awarded": <int>, "max": 4, "justification": "<string>"}}
  ],
  "total_score": <int between 0 and 20>,
  ...
  "improvements": "<what the student could improve>"
    ...
    def score(
        ...
        if raw_response == "__OLLAMA_UNAVAILABLE__":
